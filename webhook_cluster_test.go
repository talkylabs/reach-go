...
import (
	...
	StudioV2 "github.com/talkylabs/reach-go/rest/studio/v2"
)
...
func createValidationServer(channel chan bool) *http.Server {
	...
	server.Handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		signatureHeader := r.Header["X-Reach-Signature"]
		r.ParseForm()
		params := make(map[string]string)
		for k, v := range r.PostForm {
			params[k] = v[0]
		}
		requestValidator := reach.NewRequestValidator(authToken)
		if len(signatureHeader) != 0 {
			channel <- requestValidator.Validate(url, params, r.Header["X-Reach-Signature"][0])
		} else {
			channel <- false
		}
	})
