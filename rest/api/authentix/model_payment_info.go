...
// PaymentInfo Information related to the digital payment to authenticate. It is required when `usedForDigitalPayment` is true. It is ignored otherwise.
...
func (response *PaymentInfo) UnmarshalJSON(bytes []byte) (err error) {
	raw := struct {
		Payee string `json:"payee"`
	...
	*response = PaymentInfo{
	...
	responseAmount, err := client.UnmarshalFloat32(&raw.Amount)
	if err != nil {
		return err
	}
